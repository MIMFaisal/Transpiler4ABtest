...
async function createPrompt() {
  ...
  const testInfo = await inquirer.prompt([
    ...
    },
    {
      ...
      source: async (answersSoFar, input) => {
        const filteredList = fuzzy.filter(
          input,
          getFiles(`./src/${answersSoFar.newClient || answersSoFar.client}`)
        );
        let mathces = filteredList.map((el) => el.original);
        ...
        if (!input)
          mathces = [
            ...getFiles(
              `./src/${answersSoFar.newClient || answersSoFar.client}`
            ),
          ];
      ...
      when: (answers) =>
        !answers.newClient ||
        (answers.newClient && getFiles(`./src/${answers.newClient}`).length),
    },
    {
      type: "input",
      name: "newExperiment",
      ...
      when: (answers) =>
        !answers.client ||
        answers.client.includes("Create") ||
        answers.experiment.includes("Create"),
    },
    {
      type: "autocomplete",
      name: "variation",
      ...
      source: async (answersSoFar, input) => {
        const filteredList = fuzzy.filter(
          ...
          getFiles(
            `./src/${answersSoFar.newClient || answersSoFar.client}/${
              answersSoFar.newExperiment || answersSoFar.experiment
            }`
          )
        );
      ...
      when: (answers) =>
        !answers.newExperiment ||
        (answers.newExperiment &&
          getFiles(
            `./src/${answers.newClient || answers.client}/${
              answers.newExperiment
            }`
          ).length),
    },
    {
      ...
      when: (answers) =>
        answers.newExperiment || answers.variation.includes("Create"),
    },
    {
      type: "input",
      name: "siteLink",
      message:
        "Paste Site Link Once to enable auto redirection or Press Enter to Continue:",
...
export { getFiles, sharedJsContent, createFile, createPrompt };
